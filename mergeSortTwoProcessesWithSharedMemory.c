...
int main(int argc, char* argv[]) {
    ...
    if (block.size > SPLIT) {
        ...
        int* temp = mmap(NULL, second_block.size  * sizeof(int), PROT_READ | PROT_WRITE, MAP_ANONYMOUS | MAP_SHARED, -1, 0);
        pid_t cpid;
        if ((cpid = fork()) < 0) {
            ...
            memcpy(temp, (second_block).data, second_block.size * sizeof(int));
            ...
            wait(NULL);
            //wait(0);
            memcpy((second_block).data, temp, second_block.size * sizeof(int));
